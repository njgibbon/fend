...
import (
	...
	"github.com/njgibbon/fend/fendscan"
	"gopkg.in/yaml.v2"
)
...
func main() {
	...
	passed, failed, skippedDirs, skippedFiles, errors, err :=
		fendscan.FendScan(fendConfig.Skip.File, fendConfig.Skip.FileAll, fendConfig.Skip.Dir, fendConfig.Skip.DirAll, fendConfig.Skip.Extension, ".")
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
